...
function metro_home_genesis_meta() {
	...
	if ( is_active_sidebar( 'home-top' ) || is_active_sidebar( 'home-middle-left' ) || is_active_sidebar( 'home-middle-right' ) || is_active_sidebar( 'home-bottom' ) ) {
		...
		add_action( 'genesis_loop', 'metro_homepage_widgets',1 );

		//add blog link
		add_action( 'genesis_loop', 'add_blog_link',100 );
...
function add_blog_link()
{
	echo '<div class="archive-pagination pagination"><ul>
<li><a href="/blog" class="home-blog-link">View All Blog Posts</a></li>
</ul> </div>';
}
